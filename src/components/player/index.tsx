...
const Player = () => {
  ...
  const repeat = useCallback(() => {
    const time = Math.round(audioRef?.current?.currentTime || 0);
    dispatch(handleCurrentTime(time));
    dispatch(handleTimeProgress(time));
  ...
  const handleProgressChange = (value: number[]) => {
    if (audioRef?.current?.currentTime) {
      ...
      dispatch(handleCurrentTime(value[0]));
    }
  };

  const onLoadedMetadata = () => {
    console.log('onLoadedMetadata');
    dispatch(handleIsLoadingMetadata(false));
    setShowPreview(true);
    ...
    dispatch(handleDuration(seconds));
  };
  ...
  return (
    <div>
      <PlayerPreview show={showPreview} />
      <div
        className={`${
